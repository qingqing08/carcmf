...
//            echo $keyword;
            ...
            $data['data'] = Db::table('c_product')->where($where)->page($page)->limit($limit)->order('c_time' , 'desc')->select();
            ...
            $data['count'] = Db::table('c_product')->where($where)->count('id');
            $data['code'] = 0;
            $data['msg'] = '';
            return json($data);
        }
...
//            $data['image'] = str_replace('/warehouse/hisiphp/public' , '' , $data['image']);
            ...
            $res = Db::table("c_product")->insert($data);
        ...
        //产品分类----list
        $category_list = Db::table('c_product_category')->select();
...
//            $data['image'] = str_replace('/warehouse/hisiphp/public' , '' , $data['image']);
            ...
            $res = Db::table("c_product")->update($data);
        ...
        $id = get_num();
        $info = Db::table("c_product")->where('id' , $id)->find();
