...
<form class="layui-form" action="<?php echo url(); ?>" method="post">
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <select name="la_id" class="field-type" type="select" lay-filter="type">
                ...
                <?php if(is_array($laboratory_list) || $laboratory_list instanceof \think\Collection || $laboratory_list instanceof \think\Paginator): $i = 0; $__LIST__ = $laboratory_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($vo['id'] == $info['la_id']): ?>
                <option value="<?php echo htmlentities($vo['id']); ?>" selected="selected"><?php echo htmlentities($vo['laboratory_name']); ?></option>
                <?php else: ?>
                <option value="<?php echo htmlentities($vo['id']); ?>"><?php echo htmlentities($vo['laboratory_name']); ?></option>
                <?php endif; ?>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    ...
    <div class="layui-tab-item layui-show">
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" class="layui-input" value="<?php echo htmlentities($info['name']); ?>" name="name" lay-verify="required" autocomplete="off" placeholder="请输入视频名称">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline upload">
            ...
            <input type="text" style="display: none;" class="upload-input" id="file" name="download" value="<?php echo htmlentities($info['download']); ?>">
        </div>
    </div>
...
<script>
    layui.use(['layedit','upload'], function(){
        ...
        upload.render({
            elem: '#video'
            ,url: '<?php echo url("admin/laboratory_video/upload_file?type=video"); ?>' //改成您自己的上传接口
            ,accept: 'file' //普通文件
            ,done: function(res){
                layer.msg('上传成功');
        ...
        upload.render({
            elem: '#test3'
            ,url: '<?php echo url("admin/laboratory_video/upload_file?type=file"); ?>' //改成您自己的上传接口
            ,accept: 'file' //普通文件
            ,done: function(res){
                layer.msg('上传成功');
                $("#file").val(res.path);
        ...
        layedit.set({
            uploadImage: {
                url: '<?php echo url("admin/laboratory_video/upload?group=product&thumb=200&water=no"); ?>' //接口url
            }
        });
